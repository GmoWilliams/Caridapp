...
struct APIRequest5 {
    ...
    init (endpoint:String){
        let baseURL = "https://caridapp.herokuapp.com/historyVerify/\(endpoint)/"
        guard let resourceURL = URL (string: baseURL) else {fatalError()}
        
        self.resourceURL = resourceURL
    }
    
    func save(_ dataToSave:DonationVerification, completion: @escaping(Result<DonationVerification, APIError5>) -> Void){
        do{
